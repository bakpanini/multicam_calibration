...
def multicam_capture():
    ...
    while True:
        ret_1, img1 = cam_1.read()
        ret_2, img2 = cam_2.read()

        img1 = cv.resize(img1,(1280,720),fx=0,fy=0, interpolation = cv.INTER_CUBIC)
        img2 = cv.resize(img2,(1280,720),fx=0,fy=0, interpolation = cv.INTER_CUBIC)
        ...
        # axes[0].axhline(450)
        # axes[1].axhline(450)
        # plt.suptitle("Undistorted images or Rectified images")
        # plt.show()
        ...
        disparity_SGBM_unnorm = disparity_SGBM
        disparity_SGBM_unnorm_int = np.uint8(disparity_SGBM_unnorm)
        #cv.imshow("Disparity", disparity_SGBM_unnorm)
